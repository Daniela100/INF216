...
function create () {
    ...
    ah = this.sound.add('ah', {volume: 0.1,loop: false});
    ...
    this.anims.create({ key: 'left',
     ...
     this.anims.create({ key: 'deadB',
        frames: this.anims.generateFrameNumbers('lutadorB', { start: 22, end: 23 }),
        frameRate: 10});
    ...
    var collider = this.physics.add.collider(lutA, lutB, function (lutA, lutB) {
        if (lutB.anims.currentAnim.key == 'kick' && lutA.anims.currentAnim.key== 'idleA') {     
           ...
           ah.play();
           valorVidaA -= VIDA_PERDIDA;
           console.log('VIDA_A: ' + valorVidaA)
           if(valorVidaA <= 0) {
        ...
        if (stateFragil.indexOf(lutB.anims.currentAnim.key)>-1   && golpesA.indexOf(lutA.anims.currentAnim.key) >-1) {     
           ...
           ah.play();
           valorVidaB -= VIDA_PERDIDA;
           console.log('VIDA_B: ' + valorVidaB)
           if(valorVidaB <= 0) {
               isDeadB = true;
...
function update (){
    ...
    if (lutB.anims.currentAnim.key != 'idle' && lutB.anims.currentAnim.key != 'deadB') {
